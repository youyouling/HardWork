...
import com.youyou.myapplication.R;
...
public class CustomTitleView extends View {
    ...
    public CustomTitleView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R
                .styleable.CustomTitleView, defStyleAttr, 0);
        int n = a.getIndexCount();
        for (int i = 0; i < n; i++) {
            int attr = a.getIndex(i);
            switch (attr) {
                case R.styleable.CustomTitleView_titleTextCorlor:
                    //默认的颜色设置为黑色
                    mTitleTextColor = a.getColor(attr, Color.BLACK);
                    break;
                case R.styleable.CustomTitleView_titleTextSize:
                    //默认设置为16sp,TypeValue也可以把sp转换成px
